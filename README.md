...
2. Sync the environment: `uv sync`.
    - If you want to install the dependencies for the Gradio UI, run `uv sync --extra apps` or `uv sync --extra benchmarks`.
...
4. Install our custom-built `nvblox` bindings (Bypasses wheel filename checks):
```bash
...
*If you need to build `nvblox` from source for a different OS/CUDA version, please see `docs/ADVANCED_NVBLOX.md` and the [official nvblox documentation](https://nvidia-isaac.github.io/nvblox/public/pages/installation.html).*
